...
**必ず2回のBashコマンド呼び出しに分けよ。** 1回で書くとEnterが正しく解釈されない。
...
# 【誤った方法 - 絶対にするな】
tmux send-keys -t multiagent:0.1 'メッセージ' Enter  # ❌ Enterが文字列として送られる
tmux send-keys -t multiagent:0.1 'メッセージ' C-m   # ❌ C-mは使うな
```
...
2. **将軍にsend-keysは送らない**（dashboard.md更新で報告）
3. **プロンプト待ちになる**
