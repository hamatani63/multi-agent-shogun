...
※ これはコンパクション復帰とは異なる。セッション開始 = Gemini CLIを新規に立ち上げた時の手順である。

1. **Memory MCPを確認せよ**: まず Memory MCPの `read_graph` を実行し、保存されたルール・コンテキスト・禁止事項を確認せよ。記憶の中に汝の行動を律する掟がある。これを読まずして動くは、刀を持たずに戦場に出るが如し。
...
> - **セッション開始**: Gemini CLIの新規起動。白紙の状態からMemory MCPでコンテキストを復元する
...
この手順は GEMINI.md（自動読み込み）のみで完結する。instructions/ashigaru.md は初回復帰時には読まなくてよい（2タスク目以降で必要なら読む）。
...
Layer 4: Session（揮発・コンテキスト内）
  └─ GEMINI.md（自動読み込み）, instructions/*.md
...
Gemini CLIでは、MCPツールは `~/.gemini/settings.json` で設定される。
設定済みのMCPサーバーのツールは自動的に利用可能となる。
...
> コンパクション後に不安な場合は Memory MCP の read_graph で確認せよ。
...
|------|----------------|------------|
| 設定ファイル | `~/.claude/` | `~/.gemini/settings.json` |
| 指示書 | `CLAUDE.md` | `GEMINI.md` |
| MCP設定 | `claude mcp add` | `~/.gemini/settings.json` |
| 権限管理 | `--dangerously-skip-permissions` | Trusted Folders |
| コンテキスト | ~200Kトークン | ~1Mトークン |

### Gemini CLI固有の設定

- **設定ファイル**: `~/.gemini/settings.json`
- **MCP設定**: 同ファイル内の `mcpServers` セクション
- **権限管理**: `trustedFolders` で信頼済みディレクトリを設定
